<?php
// 1. Démarrage de session et connexion BDD
session_start();
require_once '../../database.php';

// On regarde si la personne connectée est un admin (pour afficher ou cacher les boutons de modification)
$est_admin = 0;
if (isset($_SESSION['est_admin']) && $_SESSION['est_admin'] == 1) {
    $est_admin = 1;
}

// 2. Récupération des catégories (pour créer les boutons de filtre en haut)
$requeteTypes = $pdo->query("SELECT * FROM TYPE_PRESTATION");
$types = $requeteTypes->fetchAll();

// 3. Logique de Filtrage (La partie la plus importante)
// Si l'utilisateur a cliqué sur un bouton de filtre (l'URL contient ?type=...)
if (isset($_GET['type'])) {
    
    $id_filtre = $_GET['type'];
    
    // Requête AVEC filtre (Clause WHERE)
    ...
    // Par défaut : l'utilisateur vient d'arriver, on affiche TOUT
    $sql = "SELECT * FROM PRESTATION 
            INNER JOIN TYPE_PRESTATION ON PRESTATION.id_type = TYPE_PRESTATION.id_type";
...
// On récupère le résultat final (filtré ou non)
